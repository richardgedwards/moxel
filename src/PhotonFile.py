...
from epax_x1 import EPAX_X1

if __name__=="__main__":
    photon = PhotonFile('./resource/newfile.photon')
    photon.readFile()

    photon.filename = 'epaxtest.photon'

    c = 155e-3
    dz = 50e-6
    exposure_time_sec = 6
    exposure_bottom_sec = 35
    n_bottom_layers =
    px, py, pz = 1440, 2560, int(c/dz)
    dx = dy = 47.25e-6
    a = dx*px
    b = dy*py
    
    photon.Header['Bed X (mm)'] = photon.float_to_bytes(a/1e-3)
    photon.Header['Bed Y (mm)'] = photon.float_to_bytes(b/1e-3)
    photon.Header['Bed Z (mm)'] = photon.float_to_bytes(c/1e-3)
    photon.Header['Layer height (mm)'] = photon.float_to_bytes(dz/1e-3)
    photon.Header['Resolution X'] = photon.int_to_bytes(px)
    photon.Header['Resolution Y'] = photon.int_to_bytes(py)
    photon.Header['Exp. time (s)'] = photon.float_to_bytes(exposure_time_sec)
    photon.Header['Exp. bottom (s)'] = photon.float_to_bytes(exposure_bottom_sec)
    photon.Header['# Bottom Layers'] = photon.int_to_bytes(n_bottom_layers)

    print('Bed X (mm):', photon.bytes_to_float(photon.Header['Bed X (mm)']))
    print('Bed Y (mm):', photon.bytes_to_float(photon.Header['Bed Y (mm)']))
    print('Bed Z (mm):', photon.bytes_to_float(photon.Header['Bed Z (mm)']))
    print('padding0:', photon.bytes_to_int(photon.Header['padding0']))
    print('Layer height (mm):', photon.bytes_to_float(photon.Header['Layer height (mm)']))
    print('Exp. time (s):', photon.bytes_to_float(photon.Header['Exp. time (s)']))
    print('Exp. bottom (s):', photon.bytes_to_float(photon.Header['Exp. bottom (s)']))
    print('Off time (s):', photon.bytes_to_float(photon.Header['Off time (s)']))
    print('# Bottom Layers:', photon.bytes_to_int(photon.Header['# Bottom Layers']))
    print('Resolution X:', photon.bytes_to_int(photon.Header['Resolution X']))
    print('Resolution Y:', photon.bytes_to_int(photon.Header['Resolution Y']))
    print('Preview 0 (addr):', photon.bytes_to_int(photon.Header['Preview 0 (addr)']))
    print('Layer Defs (addr):', photon.bytes_to_int(photon.Header['Layer Defs (addr)']))
    print('# Layers:', photon.bytes_to_int(photon.Header['# Layers']))
    print('Preview 1 (addr):', photon.bytes_to_int(photon.Header['Preview 1 (addr)']))
    print('unknown6:', photon.bytes_to_float(photon.Header['unknown6']))
    print('Proj.type-Cast/Mirror:', photon.bytes_to_int(photon.Header['Proj.type-Cast/Mirror']))
    print('padding1:', photon.bytes_to_int(photon.Header['padding1']))
